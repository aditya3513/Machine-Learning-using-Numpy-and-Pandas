	...
	for line in X_train:
		...
		if wordId in vocab:
			...
			if wordId in classList[classKey]:
				classList[classKey][wordId] += count
			else:
				classList[classKey][wordId] = count
...
def ConditionalProbDict(vocab, classDict, classList):
	...
	for class_i in classList:
		...
		documentCounts = []
		for c in classList[class_i]:
			documentCounts.append(int(c))

		totalCounts = sum(documentCounts)
		...
		for wordId in vocab:
			...
			if wordId in class_i:
				...
				# print("calss i = ", classList[class_i])
				# print("word id = ", wordId)
				# print("==============")
				word_count = classList[class_i][wordId]
				prob = (word_count + 1) / float(totalCounts + len(vocab))
				condProbDict[wordId] = np.log(prob)
			else:
				# in this case prob of finding word in class is zero so Numerator is (0 + 1) = 1
				prob = (0 + 1) / float(totalCounts + len(vocab))
				condProbDict[wordId] = 	np.log(prob)
		...
		for classId in condProbClassDict:
			...
			for wordId in vocab:
				...
				if wordId in test_dict[test_docId]:
					score_basic = int(condProbClassDict[classId][wordId])
				else:
					score_basic = 0
				
				score += score_basic * int(condProbClassDict[classId][wordId])
			if score > maxScore:
				maxScore = score
				bestClass = classId
