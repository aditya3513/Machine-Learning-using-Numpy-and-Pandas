...
def run(iterations, epsilon):
	...
	for train_index, test_index in kf.split(X):
		...
		'''
		showGraph = False
		if k==9 and epsilon == 0.0004:
			showGraph = True
		W, LL = train(X_train_scaled, Y_train, 0.0004, iterations, showGraph, epsilon)
		LLs.append( (sum(LL)/len(LL)) )
...
LLs = []
for e in np.arange(0.0, 1.0, 0.0001):
	LLs.append(run(max_iterations, e))
