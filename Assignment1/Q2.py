...
featureName = ["cap-shape", "cap-surface", "cap-color", "bruises", "odor",
  "gill-attachment", "gill-spacing", "gill-size", "gill-color", "stalk-shape", "drop", "stalk-surface-above-ring",
  "stalk-surface-below-ring","stalk-color-below-ring", "stalk-color-above-ring", "veil-type", "veil-color",
  "ring-number", "ring-type", "spore-print-color", "population", "habitat", "class"]

#reading CSV into a DataFrame
mushroom = pd.read_csv("mushroom.csv", names=featureName)

#drop columns with missing  value
mushroom = mushroom.drop(["drop"], axis=1)
#get all columns except one with class names, for normalization
df = mushroom.loc[:, mushroom.columns != 'class']
...
for k in range(10):
  ...
  X_train.append(train.loc[:, mushroom.columns != 'class']) # training data, attribute values
  X_test.append(test.loc[:, mushroom.columns != 'class']) # test data, attribute values

  Y_train.append(train.loc[:, mushroom.columns == 'class']) # train data, class values
  Y_test.append(test.loc[:, mushroom.columns == 'class']) # test data, class values
...
def run(threshold):
  ...
  for i in range(10):
    ...
    DTree = createDTree(dictionary, Y_train[i], 1)
